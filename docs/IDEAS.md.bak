# IDEAS.md — All the Kewl Shit for Organize
...
- Before moving files, create a manifest: `.organize-manifest.json`
- Manifest records: original path, new path, timestamp
- `organize undo` command reverts the last operation
- `organize undo --all` reverts everything in current session
...
- `organize watch ~/Downloads` — monitors folder continuously
...
- `organize batch ~/Desktop ~/Downloads ~/Documents`
...
- `organize --profile screenshots` — use a specific config
- Profiles for different workflows: Downloads cleanup, project archiving, etc.
- `organize profile list` / `organize profile create`

---

## Plugin System (v2.0)

- Go plugin interface for custom processors
...
### Date-Based Sorting
- `organize --by-date` — group by month/year
- `organize --by-date --format "2026/08"` — custom date format
...
- Shortcuts.app integration — run organize from Apple Shortcuts

### CLI Integration
- Pipe support: `ls | organize --stdin`
...
- Homebrew tap: `brew install organize-tui/organize`
...
- `organize --retro` — green-on-black CRT terminal aesthetic
- `organize --minimal` — ultra-clean, no colors, just text
